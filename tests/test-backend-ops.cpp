...
struct test_concat : public test_case {
    ...
    const int dim;
    const int v; // view (bits 0/1: non-contiguous first 3 dims; bits 2/3: strided dim 3)
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    for (int v : { 0, 4, 8, 12 }) {
        for (int dim : { 0, 1, 2, 3 }) {
            test_cases.emplace_back(new test_concat(GGML_TYPE_Q8_0, {128, 12, 13, 14}, dim == 0 ? 256 : 7, dim, v));
        }
    }
        ...
        for (bool with_norm : {false, true}) {
            for (bool bias_probs : {false, true}) {
                for (float scale_w : {0.0f, 2.0f}) {
                    ...
                    test_cases.emplace_back(new test_topk_moe({160, 22, 1, 1}, 6, with_norm, bias_probs, gate, scale_w));
