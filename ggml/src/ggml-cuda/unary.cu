...
// swiglu_clamp: silu(min(gate, limit)) * clamp(up, -limit, limit)

static __global__ void swiglu_clamp_kernel(const float * x, const float * g, float * dst, const int64_t k, const int64_t n, const int64_t o0, const int64_t o1, float limit) {
    const int64_t i = int64_t(blockDim.x)*blockIdx.x + threadIdx.x;
    ...
    const float xi = fminf(x[j0], limit);
    const float gi = fminf(fmaxf(g[j1], -limit), limit);

    dst[i] = (xi / (1.0f + expf(-xi))) * gi;
}
...
void ggml_cuda_op_swiglu_clamp(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int64_t num_blocks = (k + CUDA_GLU_BLOCK_SIZE - 1) / CUDA_GLU_BLOCK_SIZE;
    swiglu_clamp_kernel<<<num_blocks, CUDA_GLU_BLOCK_SIZE, 0, stream>>>(src0_p, src1_p, (float *) dst_d, k, nc, src0_o / sizeof(float), src1_o / sizeof(float), limit);
}
